...
class User extends Authenticatable implements MustVerifyEmail, JWTSubject, IsMessageable
{
    use SoftDeletes;
    use Notifiable;
    ...
    public function getUserCompanyName($type = null): string
    {
        $company = $this->company ? $this->company->name : '';
        if ($type == 'admin'){
            $company = !empty($company) ? "(Company: $company)" : '';
        } else {
            return $company;
        }
        return $company;
    ...
    public function hasWishList($moduleType, $moduleEntityId)
    {
        return $this->hasOne(UserWishList::class, 'user_id', 'id')
            ->where('object_model', $moduleType)
            ->where('object_id', $moduleEntityId);
    }
    ...
    public function useFeature($slug, $counter = -1)
    {
    ...
    public function checkJobPlan()
    {
        ...
        return $this->canUseFeature(PlanFeature::JOB_CREATE, $this->company->jobs());
    }

    public function canUseFeaturedJobPlan()
    {
        if (!setting_item('job_require_plan')) return true;
        return $this->canUseFeature(PlanFeature::JOB_SPONSORED);
    }

    public function checkEquipmentPlan()
    {
        return $this->canUseFeature(PlanFeature::EQUIPMENT_CREATE);
    }

    public function canUseFeaturedEquipmentPlan()
    {
        return $this->canUseFeature(PlanFeature::JOB_SPONSORED);
    }
    ...
    public function canUseFeature($slug)
    {
        ...
        if (!$userPlan->is_valid) return false;
        ...
        return $features[$slug] > 0;
    }

    public function checkJobPostingMaximum()
    {
        $maxJobs = setting_item('job_company_max_post_jobs');
        if (!$maxJobs || setting_item('job_require_plan')) return true;

        $countService = $this->company->jobs()->count('id');
        if ($countService >= $maxJobs) {
            return false;
        }
        return true;
    }
    ...
    {
        $maxGigs = setting_item('gig_max_posts');
        if (!$maxGigs) return true;

        $countService = $this->gigs->count('id');
        if ($countService >= $maxGigs) {
            return false;
        }
        return true;
    }
    ...
    {
        return $this->hasMany(Equipment::class, 'author_id', 'id');
    }
    ...
    public function applyPlan(Plan $plan, $price, $params = [])
    {
        if ($plan->isOneTime()) {
            $plan->apply($this, $params);
            return;
        }
        $userPlan = $this->user_plan ?: new UserPlan();
//            if(!$userPlan){
...
//                {
//                    $max_service  += $userPlan->max_service;
//                }
//            }
        $isAnnual = $params['annual'] ?? false;
        if ($isAnnual) {
            $endDate = strtotime('+ 1 year');
        } else {
            $endDate = strtotime('+ ' . $plan->duration . ' ' . $plan->duration_type);
        }
        $planFeatures = $plan->features->pluck('value', 'slug');
        $planData = $plan->toArray();
        unset($planData['content'], $planData['is_recommended']);
        $planData['is_annual'] = $isAnnual;
        $planData['features'] = $planFeatures;
    ...
    {
        $counts = $this->messageNotificationService->unreadMessagesByMessageable(Auth::id());
        $newMessagesCount = 0;
