<div class="bc-order-overview">
    ...
    <div class="order-equipment-item">
        <div class="thumb">
            @if($order->equipment->image_id)
                {!! get_image_tag($order->equipment->image_id,'full',['alt'=>$order->equipment->title]) !!}
            @endif
        </div>
        <div class="caption">
            <h3 class="equipment-title">
                <a href="{{ $order->equipment->getDetailUrl() }}" target="_blank">{{ $order->equipment->title }}</a>
            </h3>
