...
@section('content')
    <form
        action="{{route('candidate.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}"
                ...
                <div class="">
                    ...
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}
                            : {{ url( (request()->query('lang') ? request()->query('lang').'/' : '').config('candidate.candidate_route_prefix'))  }}
        ...
        jQuery(function ($) {
            new BravoMapEngine('map_content', {
                ...
                ready: function (engineMap) {
                    @if($row->location->map_lat && $row->location->map_lng)
