...
use Modules\Company\Models\CompanyCategoryTranslation as CategoryTranslation;

class CompanyCategoryController extends AdminController
{
    public function __construct()
    {
        $this->setActiveMenu('admin/module/company/category');
        parent::__construct();
    }
    ...
    public function index(Request $request)
    {
        ...
        return view('Company::admin.category.index', $data);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (empty($row)) {
            return redirect('admin/module/company/category');
        }
        $data = [
            'row'     => $row,
        ...
        return view('Company::admin.category.detail', $data);
    }
    ...
    public function store(Request $request, $id){
        ...
        if($id>0){
            $row = Category::find($id);
            if (empty($row)) {
                return redirect(route('company.admin.category.index'));
            }
        }else{
            $row = new Category();
            $row->status = "publish";
        ...
        if ($res) {
            if($id > 0 ){
                ...
                return redirect(route('company.admin.category.index'))->with('success', __('Category created') );
            }
        }
    }
