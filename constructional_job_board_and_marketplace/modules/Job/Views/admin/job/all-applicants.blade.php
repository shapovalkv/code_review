                                    ...
                                    @if($rows->total() > 0)
                                        @foreach($rows as $row)
                                            <tr class="{{$row->status}}">
                                                ...
                                                <td>
                                                    @if( $user = (!empty($row->candidateInfo) ? $row->candidateInfo->getAuthor : $row->user))
                                                        <a href="{{ $user->isCandidate() ? route('candidate.detail', ['slug' => $row->candidateInfo->slug]) : '' }}" target="_blank">
                                                            <img src="{{ $user->getAvatarUrl() }}" style="border-radius: 50%" class="company-logo" />
                                                            {{ $user->getDisplayName() ?? ''}}
                                                        </a>
                                                    @endif
                                                </td>
