...
//                'phone' => 'required',
        ...
        $row->birthday = $request->input('birthday') ? date("Y-m-d", strtotime($request->input('birthday'))) : null;
        ...
        if ($row->save()) {
            if ($row->role_id == 3) {
                ...
                $cData->allow_search = $request->allow_search == 'hide' ? 'draft' : 'publish';
            ...
            if ($request->query("user_type") == "candidate") {
                ...
                foreach ($res as $item) {
                    if ($item->hasPermission("gig_manage")) {
                        $data[] = [
                            'id' => $item->id,
                            'text' => $item->getDisplayName() ? $item->getDisplayName() . $item->getRoleNameAttribute() . ' (#' . $item->id . ')' : $item->email . ' (#' . $item->id . ')',
                        ];
                    }
                }
            }elseif ($request->query("user_type") == "employer"){
                // only for employers
                foreach ($res as $item) {
                    if ($item->hasPermission("equipment_manage")) {
                        $data[] = [
                            'id' => $item->id,
                            'text' => $item->getDisplayName() ? $item->getDisplayName() . ' ' . $item->getUserCompanyName('admin') . ' ' . ' (#' . $item->id . ')' : $item->email . ' (#' . $item->id . ')',
                        ];
                    }
                }
            } else {
        ...
        if ($action == 'delete') {
            foreach ($ids as $id) {
                ...
                if (!empty($query)) {
                    ...
                    if (!empty($candidate)) {
                        if (!empty($candidate->candidatesApplies)){
                            $candidate->candidatesApplies()->delete();
                        }
                        $candidate->delete();
                    }
                }
