...
<div class="row">
    <div class="col-sm-4">
        <h3 class="form-group-title">{{__('Content Email Staff Created')}}</h3>
        <div class="form-group-desc">{{ __('Content email send to Customer or Administrator when Employer created staff account.')}}
            @foreach(\Modules\Company\Listeners\SendMailStaffCreatedListen::CODE as $item=>$value)
                <div><code>{{$value}}</code></div>
            @endforeach
        </div>
    </div>
        ...
        <div class="panel">
            <div class="panel-body">
                <div class="form-group">
                    <label>{{__("Email to customer content")}}</label>
                    <div class="form-controls">
                        <textarea name="staff_content_email_registered" class="d-none has-ckeditor" cols="30" rows="10">{{setting_item_with_lang('staff_content_email_registered',request()->query('lang')) ?? '' }}</textarea>
                    </div>
                </div>
            </div>
        </div>
