<form method="get" action="{{ (!empty($category) || !empty($location)) ? route('job.search') : request()->fullUrl() }}">
    ...
    @if($job_sidebar_search_fields)
        @foreach($job_sidebar_search_fields as $key => $val)
            ...
            @if(request()->get('_layout') == 'v2' && in_array($val['type'], ['location', 'keyword', 'category']))
                @continue
            @endif
    ...
    <div class="wrapper-submit flex-middle col-xs-12 col-md-12">
        ...
        @endif
        <button type="submit" class="theme-btn btn-style-ten">{{ \Illuminate\Support\Facades\Route::currentRouteName() === 'job.search.practicum' ? __("Find Practicum Sites") : __("Find Jobs") }}</button>
    </div>
</form>
