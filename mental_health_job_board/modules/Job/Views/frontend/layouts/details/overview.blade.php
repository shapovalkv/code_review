        ...
        <li>
            ...
            <span>
                @if(empty($row->experience) || (float)$row->experience < 1)
                    {{ __("Fresh") }}
                @else
                    {{ $row->experience }} {{ $row->experience_type ? ($row->experience > 1 ? \Illuminate\Support\Str::plural($row->experience_type) : \Illuminate\Support\Str::singular($row->experience_type)) : '' }}
                @endif
            </span>
        </li>
