...
{{--            @endif--}}
            @if($row->company->phone && \Illuminate\Support\Facades\Auth::user())
                @php
            ...
            @endif
            @if($row->company->email && \Illuminate\Support\Facades\Auth::user())
                @php
            ...
            @endif
            @if($row->company->offices()->exists())
                <li>{{__("Location(s)")}}: <span>
                           @foreach($row->company->offices()->orderBy('is_main', 'desc')->get() as $office)
        ...
        @if($row->company->website)
            <div class="btn-box"><a href="{{ ($row->company->website) }}" class="theme-btn btn-style-three" target="_blank">{{__('Company Website')}}</a></div>
        @endif
    </div>
</div>
@endif
