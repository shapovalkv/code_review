    ...
    <div class="auto-container">
        <div class="cotnent-box">
            <div class="title-box wow fadeInUp" data-wow-delay="500ms">
                <h3>{!! @clean($title) !!}</h3>
                <div class="text" style="color: #FFFFFF">{{ $sub_title }}</div>
...
{{--                        @else--}}
                            <div class="form-group col-lg-3 col-md-12 col-sm-12 location banner-category">
                                <span class="icon flaticon-map-locator"></span>
                                <select class="bc-select2" name="location">
                                    <option value="">{{ __("All Cities") }}</option>
                                    @php
                                        $location_id = $location_style === 'autocomplete' ? ((int)request()->get('location')) : null;
                                        $traverse = function ($locations, $prefix = '') use (&$traverse, $location_id) {
                                            foreach ($locations as $location) {
                                                $translate = $location->translateOrOrigin(app()->getLocale());
                                                printf("<option value='%s' %s>%s</option>", $location->id, $location_id === $location->id ? 'selected' : '', $prefix . ' ' . $translate->name);
                                                $traverse($location->children, $prefix . '-');
                                            }
                                        };
                                        $traverse($list_locations);
...
{{--                        @endif--}}
                        ...
                        <div class="form-group col-lg-3 col-md-12 col-sm-12 category banner-category">
                            ...
                            <select class="bc-select2" name="categories[]">
                        ...
                        <div class="form-group col-lg-2 col-md-12 col-sm-12 text-right">
                            <button type="submit" class="theme-btn btn-style-ten">{{ __('Find Jobs') }}</button>
                        </div>
                    </div>
                </form>
            </div>
