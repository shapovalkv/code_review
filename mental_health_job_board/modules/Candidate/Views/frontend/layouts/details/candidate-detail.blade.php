            ...
            <div class="my_resume_eduarea">
                @foreach($row->education as $oneData)
                    ...
                    <div class="resume-block">
                        <div class="inner">
                            <!-- $oneData['location'] is Name of School/Institution from Candidate profile page -->
...
{{--                                    <span>{{@$oneData['location']}}</span>--}}
                                ...
                                <div class="edit-box">
                                    <span class="year">{{@$oneData['from']}} - {{$oneData['to'] ?? 'till now'}}</span>
                                </div>
                                @if(!empty($oneData['diploma']))
                                <div class="text ml-2">
                                    {{__('Diploma/Certificate')}}: {{$oneData['diploma']}}
                                </div>
                                @endif
                            </div>
            ...
            @foreach($row->experience as $oneData)
                ...
                <div class="resume-block">
                    <div class="inner">
                        ...
                        <div class="text">{!! nl2br(strip_tags(@$oneData['description'])) !!}</div>
{{--                        <div class="text">{!! nl2br(strip_tags(@$oneData['information'])) !!}</div>--}}
                    </div>
                </div>
            @endforeach
