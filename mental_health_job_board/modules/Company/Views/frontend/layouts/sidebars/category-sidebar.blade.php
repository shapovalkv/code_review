<div class="filters-outer">
    <form method="GET" action="{{ route('companies.index') }}" class="search-company bravo_form_filter">
        ...
        <div class="wrapper-submit flex-middle col-xs-12 col-md-12">
            <button class="btn-submit theme-btn btn-style-ten" type="submit">{{__("Find Employers")}}</button>
        </div>
    </form>
    <!-- Call To Action -->
    @php
    ...
    @if(!empty($company_sidebar_cta->title))
        <div class="call-to-action-four">
            ...
            @if(!empty($company_sidebar_cta->button->url))
                <a href="{{ ($company_sidebar_cta->button->url) }}" class="theme-btn btn-style-one bg-blue"><span
                        class="btn-title">{{ $company_sidebar_cta->button->name ?? __("Start Recruiting Now") }}</span></a>
            @endif
            <div class="image"
                 style="background-image: url({{ !empty($company_sidebar_cta->image) ? \Modules\Media\Helpers\FileHelper::url($company_sidebar_cta->image, 'full') : '' }});"></div>
        </div>
    @endif
    <!-- End Call To Action -->
