    ...
    <div class="sidebar-widget company-widget">
        <div class="widget-content">
            <div class="company-title">
                ...
                @if($row->job_count > 0)
                    <a href="#"
                       class="company-link">{{ __("Open Jobs – :count",["count"=>number_format($row->job_count)]) }}</a>
                @endif
            </div>
                    ...
                    <li>{{__("Social media")}}:
                        <div class="social-links">
                            ...
                            @if(!empty($Social_media['twitter']))
                                <a href="{{ $Social_media['twitter'] }}"><i class="fab fa-x-twitter"></i></a>
                            @endif
            ...
            @if(!empty($row->website))
                <div class="btn-box"><a rel="nofollow" target="_blank" href="{{ $row->website }}"
                                        class="theme-btn btn-style-three">{{ $row->website }}</a></div>
            @endif
        </div>
    </div>
    @if(!empty($offices))
