@php
$requirements = $order->Marketplace->requirements;
        ...
        <div class="bc-list-requirements">
            @foreach($requirements as $key => $val)
                <div class="requirement-item">
                    ...
                    <div class="r-right">
                        @if($order->status == \Modules\Marketplace\Models\MarketplaceOrder::INCOMPLETE)
                            <input type="hidden" name="requirements[{{$key}}][content]" value="{{ $val['content'] ?? '' }}" >
                        @endif
                        <h4 class="question">{{ $val['content'] ?? '' }} @if($order->status == \Modules\Marketplace\Models\MarketplaceOrder::INCOMPLETE && !empty($val['required'])) <span class="text-danger">*</span> @endif</h4>
                            @if($order->status == \Modules\Marketplace\Models\MarketplaceOrder::INCOMPLETE)
