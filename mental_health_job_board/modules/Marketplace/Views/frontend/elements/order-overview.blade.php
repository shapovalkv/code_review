<div class="bc-order-overview">
    ...
    <div class="order-Marketplace-item">
        <div class="thumb">
            @if($order->Marketplace->image_id)
                {!! get_image_tag($order->Marketplace->image_id,'full',['alt'=>$order->Marketplace->title]) !!}
            @endif
        </div>
        <div class="caption">
            <h3 class="Marketplace-title">
                <a href="{{ $order->Marketplace->getDetailUrl() }}" target="_blank">{{ $order->Marketplace->title }}</a>
            </h3>
