    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events:{
                    url:"{{route('Marketplace.vendor.availability.loadDates')}}",
                    extraParams:{
                        id:lastId,
                    }
                },
                ...
                eventClick:function (info) {
                    var form = Object.assign({},info.Marketplace.extendedProps);
                    form.start_date = moment(info.Marketplace.start).format('MM/DD/YYYY');
                    form.end_date = moment(info.Marketplace.start).format('MM/DD/YYYY');
                    console.log(form);
                    formModal.show(form);
                },
                eventRender: function (info) {
                    $(info.el).find('.fc-title').html(info.Marketplace.title);
                    $(info.el).find('.fc-content').attr("data-html","true").attr("title",info.Marketplace.title).tooltip({ boundary: 'window' })
                }
            });
            calendar.render();
        });
        ...
        formModal = new Vue({
            ...
            methods:{
                ...
                saveForm:function () {
                    ...
                    $.ajax({
                        url:'{{route('Marketplace.vendor.availability.store')}}',
                        data:this.form,
                        dataType:'json',
                        method:'post',
