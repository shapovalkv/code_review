...
class MediaController extends Controller
{
                ...
                if(FileHelper::isImage($file))
                {
                    if(Storage::disk('s3')->exists($file->file_path))
                    {
                        if(function_exists('proc_open')){
                            try{
                                    ImageOptimizer::optimize(Storage::disk('s3')->url($file->file_path));
    ...
    public function store(MediaStoreRequest $request)
    {
        ...
        $file->storeAs( $folder, $newFileName2 . '.' . $file->getClientOriginalExtension(),'uploads');
        $fileUrl = Storage::disk('s3')->url($check);
        // Try to compress Images
        if(function_exists('proc_open') and function_exists('escapeshellarg')){
            try {
                ImageOptimizer::optimize($fileUrl);
            } catch (\Exception $exception) {
        ...
        if ($check) {
            try {
                ...
                $fileObj->sizes = [
                    'default' => Storage::disk('s3')->url($fileObj->file_path),
...
//            'bmp',
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                if(env('APP_PREVIEW_MEDIA_LINK')){
                    ...
                    $file->full_size = FileHelper::url($file,'full',false);
                    $file->medium_size = FileHelper::url($file,'medium',false);
                }
            }
        }
    ...
    public function removeFiles(Request $request){
        ...
        $storage2 = Storage::disk('uploads');
        if(!empty($files->count())){
            foreach ($files as $file){
                $storage->delete($file->file_path);
                $storage2->delete($file->file_path);
