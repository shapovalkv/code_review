...
<head>
    ...
    @if($favicon)
        ...
        @endphp
        @if(!empty($file))
            <link rel="icon" type="{{$file['file_type']}}" href="{{asset('uploads/'.$file['file_path']) . '?_ver=' . config('app.asset_version') }}" />
        @else:
            <link rel="icon" type="image/png" href="{{url('images/faviconMHC.jpg'). '?_ver=' . config('app.asset_version')}}" />
        @endif
    @endif
    ...
    <link href="{{ asset('module/superio/css/responsive.css?_ver='.config('app.asset_version')) }}" rel="stylesheet">
