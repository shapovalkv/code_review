        ...
        <div class="auto-container">
            <div class="upper-box">
                ...
                <h4>
                    @switch($row->status)
                        @case('completed')
                            {{__('Congratulations! Your post is now live!')}}
                        @break
                        @default
                    ...
                    @switch($row->status)
                        @case('completed')
                            <a href="{{ route('marketplace.detail', ['slug' => $announcement->slug]) }}" class="theme-btn btn-style-one bg-blue">{{ __("View your Post") }}</a>
                        @break
                        @default
