                ...
                <div class="row">
                    ...
                    <div class="form-group col-lg-4 col-md-12 col-sm-12">
                        ...
                        <input type="text" name="s" value="{{ request()->input('s') }}"
                               placeholder="{{ __("MarketplaceUser title...") }}">
                    </div>

                    <!-- Form Group -->
                    @php $location_search_style = setting_item('marketplace_user_location_search_style') @endphp
                    @if($location_search_style == 'autocomplete')
                        ...
                        <div class="form-group col-lg-3 col-md-12 col-sm-12 location smart-search">
                            <input type="text" class="smart-search-location parent_text form-control"
                                   placeholder="{{__("Start Typing")}}" value="{{ $location_name }}"
                    ...
                    @else
                        <div class="form-group col-lg-3 col-md-12 col-sm-12 location bc-select-has-delete">
                            ...
                            <select class="chosen-select" name="location">
                                <option value="">{{ __("Start Typing") }}</option>
                                @if(!empty($list_locations))
                                    @foreach($list_locations as $location)
                                        ...
                                        <option value="{{ $location->id }}"
                                                @if($location->id == request()->get('location')) selected @endif >{{ $translate->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <select class="bc-select2" name="category">
                            ...
                            @if(!empty($list_categories))
                                @foreach($list_categories as $cat)
                                    ...
                                    @endphp
                                    <option value="{{ $cat->id }}"
                                            @if($cat->id == request()->get('category')) selected @endif >{{ $translate->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    ...
                    <div class="form-group col-lg-2 col-md-12 col-sm-12 text-right">
                        <button type="submit" class="theme-btn btn-style-one">{{__('Find MarketplaceUsers')}}</button>
                    </div>
                </div>
            </form>
        </div>
...
{{--                                                        @if($skill->id == request()->get('skill')) selected @endif >{{ $translate->name }}</option>--}}
{{--                                            @endforeach--}}
{{--                                        </select>--}}
{{--                                    </div>--}}
                                    ...
                                    <div class="form-group">
                                        <select class="chosen-select" name="date_posted" onchange="this.form.submit()">
                                            <option @if(request()->get('date_posted') == "all") selected
                                                    @endif value="all">{{ __("Date Posted") }}</option>
                                            <option @if(request()->get('date_posted') == "last_hour") selected
                                                    @endif value="last_hour">{{ __("Last Hour") }}</option>
                                            <option @if(request()->get('date_posted') == "last_1") selected
                                                    @endif value="last_1">{{ __("Last 24 Hours") }}</option>
                                            <option @if(request()->get('date_posted') == "last_7") selected
                                                    @endif value="last_7">{{ __("Last 7 Days") }}</option>
                                            <option @if(request()->get('date_posted') == "last_14") selected
                                                    @endif value="last_14">{{ __("Last 14 Days") }}</option>
                                            <option @if(request()->get('date_posted') == "last_30") selected
                                                    @endif value="last_30">{{ __("Last 30 Days") }}</option>
                                        </select>
                                    </div>

                                    <div class="form-group">
                                        <select class="chosen-select" name="experience_year"
                                                onchange="this.form.submit()">
                                            <option value="">{{ __("Choose an experience") }}</option>
                                            <option @if(request()->get('experience_year') == "fresh") selected
                                                    @endif value="fresh">{{ __("Fresh") }}</option>
                                            <option @if(request()->get('experience_year') == "1") selected
                                                    @endif value="1">{{ __("1 Year") }}</option>
                                            <option @if(request()->get('experience_year') == "2") selected
                                                    @endif value="2">{{ __("2 Years") }}</option>
                                            <option @if(request()->get('experience_year') == "3") selected
                                                    @endif value="3">{{ __("3 Years") }}</option>
                                            <option @if(request()->get('experience_year') == "4") selected
                                                    @endif value="4">{{ __("4 Years") }}</option>
                                            <option @if(request()->get('experience_year') == "5") selected
                                                    @endif value="5">{{ __("5 Years") }}</option>
                                        </select>
                                    </div>

                                    <div class="form-group">
                                        <select class="chosen-select" name="education_level"
                                                onchange="this.form.submit()">
                                            <option value="">{{ __("Choose an education level") }}</option>
                                            <option @if(request()->get('education_level') == "certificate") selected
                                                    @endif value="certificate">{{ __("Certificate") }}</option>
                                            <option @if(request()->get('education_level') == "diploma") selected
                                                    @endif value="diploma">{{ __("Diploma") }}</option>
                                            <option @if(request()->get('education_level') == "associate") selected
                                                    @endif value="associate">{{ __("Associate Degree") }}</option>
                                            <option @if(request()->get('education_level') == "bachelor") selected
                                                    @endif value="bachelor">{{ __("Bachelor Degree") }}</option>
                                            <option @if(request()->get('education_level') == "master") selected
                                                    @endif value="master">{{ __("Master’s Degree") }}</option>
                                            <option @if(request()->get('education_level') == "professional") selected
                                                    @endif value="professional">{{ __("Professional’s Degree") }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                    ...
                    @if(!empty($rows) && count($rows) > 0)
                        <div class="row">
                            ...
                            @foreach($rows as $row)
                                <div class="marketplace_user-block-four col-lg-4 col-md-6 col-sm-12">
                                    @include("MarketplaceUser::frontend.layouts.loop.item-v3")
                                </div>
                            @endforeach
                        </div>
...
{{--                                <a href="{{ ($marketplace_user_sidebar_cta->button->url) }}"--}}
{{--                                   target="_{{ $marketplace_user_sidebar_cta->button->target ?? "self" }}"--}}
{{--                                   class="theme-btn btn-style-one bg-blue">--}}
{{--                                    <span--}}
{{--                                        class="btn-title">{{ $marketplace_user_sidebar_cta->button->name ?? __("Start Recruiting Now") }}</span>--}}
{{--                                </a>--}}
{{--                            @endif--}}
{{--                            <div class="image"--}}
{{--                                 style="background-image: url({{ !empty($marketplace_user_sidebar_cta->image) ? \Modules\Media\Helpers\FileHelper::url($marketplace_user_sidebar_cta->image, 'full') : '' }});"></div>--}}
{{--                        </div>--}}
{{--                        <!-- End Call To Action -->--}}
{{--                    @endif--}}
                </div>
