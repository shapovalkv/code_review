...
@section('content')
    <form action="{{route('marketplace_user.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post" class="dungdt-form">
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{__('My Profile')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url( (request()->query('lang') ? request()->query('lang').'/' : '').config('marketplace_user.marketplace_user_route_prefix'))  }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
...
{{--            @include('Language::admin.navigation')--}}
            <div class="lang-content-box">
                <div class="row">
                    <div class="col-md-9">
                        <div class="panel">
                            <div class="panel-title"><strong>{{ __('MarketplaceUser content')}}</strong></div>
                            <div class="panel-body">
                                @csrf
                                @include('MarketplaceUser::admin/marketplace_user/form',['row'=> $row])
                            </div>
                        </div>
                                        ...
                                        <div class="control-map-group">
                                            ...
                                            <div class="g-control">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="map_lat" class="form-control" value="{{$row->map_lat ?? '34.0522'}}" onkeydown="return event.key !== 'Enter';">
                                                </div>
                                                <div class="form-group">
                                                    <label>{{__("Map Longitude")}}:</label>
                                                    <input type="text" name="map_lng" class="form-control" value="{{$row->map_lng ?? '-118.244'}}" onkeydown="return event.key !== 'Enter';">
                                                </div>
                                                <div class="form-group">
